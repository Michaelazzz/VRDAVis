...
Module.zfpDecompressUint8WASM = function (u8: Uint8Array, compressedSize: number, nx: number, ny: number, nz: number, precision: number) {
...
ctx.onmessage = (event => {
    if (event.data && Array.isArray(event.data) && event.data.length > 1) {
        ...
        if (eventName === "setid") {
            ...
            let imageData = Module.zfpDecompressUint8WASM(compressedView, eventArgs.size, eventArgs.length, eventArgs.width, eventArgs.height, eventArgs.compression);
            if (Module.debugOutput) {
                performance.mark("decompressEnd");
            }
            let outputView = new Float32Array(event.data[1], 0, eventArgs.width * eventArgs.height * eventArgs.length);
            ...
            if (Module.debugOutput) {
                ...
                const eventSize = (4e-6 * eventArgs.width * eventArgs.height * eventArgs.length);
                setTimeout(() => {
