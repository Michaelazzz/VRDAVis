...
void InitLogger(bool no_log_file, int verbosity, bool log_performance, bool log_protocol_messages_, fs::path user_directory) {
    ...
    auto console_sink = std::make_shared<spdlog::sinks::vrdavis_sink>();
    console_sink->set_pattern(VRDAVIS_LOGGER_PATTERN);
    ...
    if (!no_log_file) {
        log_fullname = (user_directory / "log/vrdavis.log").string();
        ...
        stdout_log_file_sink->set_pattern(VRDAVIS_LOGGER_PATTERN);
        console_sinks.push_back(stdout_log_file_sink);
    }

    // Create the stdout logger
    auto default_logger = std::make_shared<spdlog::logger>(VRDAVIS_LOGGER_TAG, std::begin(console_sinks), std::end(console_sinks));
...
void LogReceivedEventType(const VRDAVis::EventType& event_type) {
    if (log_protocol_messages) {
        auto event_name = VRDAVis::EventType_Name(VRDAVis::EventType(event_type));
        if (!event_name.empty()) {
...
void LogSentEventType(const VRDAVis::EventType& event_type) {
    if (log_protocol_messages) {
        auto event_name = VRDAVis::EventType_Name(VRDAVis::EventType(event_type));
        if (!event_name.empty()) {
            spdlog::debug("[protocol] ==> {}", event_name);
